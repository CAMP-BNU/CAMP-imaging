classdef StartExperiment < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function button_modifyButtonPushed(app, event)
            % disable whole user panel when editing user
            app.panel_user.Enable = "off";
            % the first project does not store data
            already_data_stored = app.project_active > 2 || ...
                (app.project_active == 2 && app.project_progress > 0);
            disable_id_modify = already_data_stored;
            if already_data_stored
                selection = uiconfirm(app.UIFigure, ...
                    '当前用户已产生行为数据存储，不推荐修改编号。请确认是否要修改编号？', ...
                    '修改确认', ...
                    'Icon', 'warning', ...
                    'Options', ["否", "是"], ...
                    'CancelOption', "否");
                if selection == "是"
                    [~] = uiconfirm(app.UIFigure, '修改编号后，请修正已存储数据中对应的被试编号。', ...
                        '修改提醒', 'Icon', 'warning', 'Options', "知道了");
                    disable_id_modify = false;
                end
            end
            CreateOrModifyUser(app, "modify", ...
                "UsersHistory", app.users_history, ...
                "User", app.user, ...
                "DisableIDModify", disable_id_modify);
        end
